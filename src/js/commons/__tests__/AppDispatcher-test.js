...
describe('AppDispatcher', function() {var AppDispatcher;
  ...
  it('waits with chained dependencies properly', function() {var payload = {},
        listener1Done = false,
        listener2Done = false,
        listener3Done = false,
        listener4Done = false,
    ...
    listener1 = function() {AppDispatcher.waitFor([index2, index4]);
    ...
    listener2 = function() {AppDispatcher.waitFor([index3]);
      expect(listener3Done).toBe(true);
      listener2Done = true;
    };
    ...
    listener3 = function() {listener3Done = true;
    };
    index3 = AppDispatcher.register(listener3);

    listener4 = function() {AppDispatcher.waitFor([index3]);
      expect(listener3Done).toBe(true);
      listener4Done = true;
    };
    index4 = AppDispatcher.register(listener4);

    runs(function() {AppDispatcher.dispatch(payload);
    });

    waitsFor(function() {return listener1Done;
    }, 'Not all subscribers were properly called', 500);

    runs(function() {expect(listener1Done).toBe(true);
      expect(listener2Done).toBe(true);
      expect(listener3Done).toBe(true);
    });
  });
